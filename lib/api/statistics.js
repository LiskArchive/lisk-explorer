...
module.exports = function (app) {
	...
	function Blocks() {
		...
		this.inspect = function (blocks, offset) {
			...
			for (let i = 0; i < blocks.length; i++) {
				...
				// @todo remove assignment by #285
				const newBlock = Object.assign({}, blocks[i], blocks[i].forged);
				const newAmount = (newBlock.forged.totalAmount + newBlock.forged.totalFee);
				...
				if (newAmount > 0) {
					...
					if (newAmount > this.best.amount) {
						// @todo remove by #285
						newBlock.id = newBlock.blockId;
						newBlock.previousBlock = newBlock.previousBlockId;
						newBlock.generatorId = newBlock.generatorAddress;
						delete newBlock.forged;
						delete newBlock.blockId;
						delete newBlock.previousBlockId;
						delete newBlock.generatorAddress;
	...
	this.getBlocks = function (error, success) {
		...
		async.doUntil(
			(next) => {
				console.log('getBlocks calls ', blocks.url(offset, limit));
				request.get({
					url: app.get('lisk address') + blocks.url(offset, limit),
					json: true,
				}, (err, resp, body) => {
					console.log('Add returns', resp.statusCode);
	...
	function Peers(locator) {
		...
		this.url = (offset, limit) =>
			setOffsetAndLimit('/peers?sort=ip:asc', offset, limit);
