...
module.exports = function (app) {
	...
	function Account() {
		...
		this.getDelegateCall = (address, cb) => {
			request.get({
		...
		this.getDelegate = (account, cb) => {
			...
			knowledge.getOrSetUsername(account, (err, username) => {
				if (username) {
					account.knowledge = { owner: username };
				} else {
		...
		this.getVotes = (account, cb) => {
			...
			return async.parallel([
				...
				_.each(account.votes, (vote) => {
					knowledge.getOrSetUsername(vote, (err2, username) => {
						if (username) {
							vote.knowledge = { owner: username };
						} else {
							vote.knowledge = null;
						}
					});
					...
					_.each(account.voters, (voter) => {
						knowledge.isUsernameCached(voter.address, (err2, cached) => {
							if (cached) {
								...
								this.getDelegateCall(voter.address, (err3, delegate) => {
									if (delegate) {
