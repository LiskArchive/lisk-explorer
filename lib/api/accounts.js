...
module.exports = function (app) {
	...
	function getDelegateCall(address, cb) {
		request.get({
			url: `${app.get('lisk address')}/delegates?address=${address}`,
			json: true,
		}, (err, response, body) => {
	...
	function Account() {
		...
		this.getAccount = (params, cb) => {
			...
			if (params.address && !this.validAddress(params.address)) {
			...
			const query = params.address ? `address=${params.address}` : `publicKey=${params.publicKey}`;
		...
		this.getDelegate = (account, cb) => {
			...
			knowledge.getOrSetKnowledge(account, (err, data) => {
		...
		this.getVotes = (account, cb) => {
			...
			return async.parallel([
				...
				_.each(account.votes, (vote) => {
					knowledge.getOrSetKnowledge(vote, (err2, data) => {
						vote.knowledge = data;
					});
				});
		...
		this.getVoters = (account, cb) => {
			...
			return async.doUntil(
				...
				(err) => {
					...
					_.each(account.voters, (voter) => {
						knowledge.isAddressCached(voter.address, (err2, cached) => {
							if (cached) {
								knowledge.getKnownAddress(voter.address, (username) => {
									if (username) voter.knowledge = { owner: username };
								});
							} else {
								getDelegateCall(voter.address, (err3, delegate) => {
									if (delegate) {
										knowledge.setKnownAddress(delegate);
										knowledge.getKnownAddress(delegate.address, (err4, username) => {
											if (username) voter.knowledge = { owner: username };
										});
									}
								});
	...
	this.getTopAccounts = (query, error, success) => {
		...
		return request.get({
			...
			if (err || response.statusCode !== 200) {
				...
				return async.forEach(accounts, (account, cb) => {
					knowledge.getOrSetKnowledge(account, (err2, data) => {
						account.knowledge = data;
						cb(null, account);
					});
				}, () => {
