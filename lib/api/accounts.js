...
module.exports = function (app) {
	...
	function Account() {
		...
		this.getMultisignaturesGroups = (account, cb) => {
			request.get({
				...
				if (response.statusCode === 404 &&
					...
					account.multisignatures = _.extend(account.multisignatures || []);
					return cb(null, account);
				}
				return cb(body.error);
			});
		...
		this.getVotes = (account, cb) => {
			...
			return request.get({
				url: `${app.get('lisk address')}/accounts?address=${account.address}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
				} else if (Array.isArray(body.data)) {
					account.votes = (body.data !== undefined &&
						body.data !== null && body.data.length > 0) ?
						body.data : null;
		...
		this.getVoters = (account, cb) => {
			...
			return request.get({
				...
				if (err || response.statusCode !== 200) {
					...
					account.voters = (body.data.voters !== undefined &&
						body.data.voters !== null && body.data.voters.length > 0) ?
						body.data.voters : null;
