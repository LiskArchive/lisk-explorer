...
module.exports = function (app) {
	...
	function Account() {
		...
		this.getAccount = (address, cb) =>
			request.get({
				...
				console.log('#28', err, body, Object.keys(response));
				if (err || response.statusCode !== 200) {
					return cb(null, err || 'Response was unsuccessful');
				}
		...
		this.getVotes = (account, cb) => {
			...
			return request.get({
				url: `${app.get('lisk address')}/accounts/delegates/?address=${account.address}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
		...
		this.getVoters = (account, cb) => {
			...
			return request.get({
				url: `${app.get('lisk address')}/delegates/voters?publicKey=${account.publicKey}`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getAccount = (params, error, success) => {
		...
		return async.waterfall([
			(cb) => {
				if (params.address) {
					account.getAccount(params.address, (a, b, c) => {
						// console.log('+++', a, b, c);
						cb(null, a, b, c);
					});
