			...
			return async.doUntil(
				...
				(err) => {
					...
					_.each(account.voters, (voter) => {
						knowledge.isUsernameCached(voter.address, (err2, cached) => {
							if (cached) {
								knowledge.getUsername(voter.address, (username) => {
									if (username) voter.knowledge = { owner: username };
								});
							} else {
								getDelegateCall(voter.address, (err3, delegate) => {
									if (delegate) {
										knowledge.setUsername(delegate);
										knowledge.getUsername(delegate.address, (err4, username) => {
											if (username) voter.knowledge = { owner: username };
										});
									}
								});
