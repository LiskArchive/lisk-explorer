...
module.exports = function (app) {
	...
	this.getStatistics = function (e, error, success) {
		...
		(err, results) => {
			...
			let statistics = {
				...
				liskVolume: _.reduce(results[0], (memo, t) => (memo + parseFloat(t.liskVolume)), 0.0).toFixed(8),
				numTrades: _.reduce(results[0], (memo, t) => (memo + parseInt(t.numTrades)), 0),
			};
	...
	var rejectCandles = function (candles, maxTime) {
		return _.reject(candles, c => moment.unix(c.timestamp).isBefore(maxTime));
	};
};
