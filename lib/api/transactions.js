...
module.exports = function (app) {
	...
	const getSenderDelegate = function (transaction, cb) {
	...
	const getRecipientDelegate = function (transaction, cb) {
		...
		return async.waterfall([
			...
			(result, cb) => {
				if (query.timestampQuery) {
					result = _.sortBy(result, o => -o.timestamp).slice(0, query.limit);
				}
				cb(null, result);
			},
			processTransactions,
