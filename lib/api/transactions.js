...
module.exports = function (app) {
	...
	const normalizeTransactionParams = async (params) => {
		...
		const liskIdPattern = /([0-9]+)L/;

		let address = coreUtils.parseAddress(params.address);
		let senderId = coreUtils.parseAddress(params.senderId);
		let recipientId = coreUtils.parseAddress(params.recipientId);

		if (address && !address.match(liskIdPattern)) address = await knowledge.getByUser(address);
		if (senderId && !senderId.match(liskIdPattern)) senderId = await knowledge.getByUser(senderId);
		if (recipientId && !recipientId.match(liskIdPattern)) {
			recipientId = await knowledge.getByUser(recipientId);
		}

		if (typeof address !== 'string' && typeof address.address === 'string') address = address.address;
		if (typeof senderId !== 'string' && typeof senderId.address === 'string') params.senderId = senderId.address;
		if (typeof recipientId !== 'string' && typeof recipientId.address === 'string') params.recipientId = recipientId.address;
