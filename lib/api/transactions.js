...
module.exports = function (app) {
    ...
    this.getTransactionsByAddress = function (query, error, success) {
        ...
        if (query.direction === 'sent') {
            directionQueries.push('senderId=' + query.address + '&and:type=0');
        } else if (query.direction === 'received') {
            directionQueries.push('recipientId=' + query.address + '&and:type=0');
        } else if (query.direction === 'others') {
            // TODO: We should also support type 7 here
            for (var i = 1; i < 7; i++) {
                directionQueries.push('senderId=' + query.address + '&and:type=' + i);
            }
        } else {
            directionQueries.push('recipientId=' + query.address + '&or:senderId=' + query.address);
        }

        async.waterfall([
            function (cb) {
                var transactionsList = [];
                directionQueries.forEach(function (directionQuery, index) {
                    request.get({
                        ...
                        if (err || response.statusCode !== 200) {
                            ...
                            if (index === directionQueries.length - 1) {
