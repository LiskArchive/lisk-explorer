...
module.exports = function (app) {
	...
	this.loadAllDelegates = function () {
	...
	function Active() {
		...
		this.getActive = function (cb) {
			request.get({
				url: `${app.get('lisk address')}/delegates/?limit=101`,
				json: true,
			}, (err, response, body) => {
				if (err || response.statusCode !== 200) {
					return cb(err || 'Response was unsuccessful');
	...
	this.getStandby = function (n, error, success) {
		...
		request.get({
			url: `${app.get('lisk address')}/delegates/?limit=${delegates.limit}&offset=${delegates.actualOffset}`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				...
				return success({
					success: true,
					delegates: parseDelegates(body.data.map(delegatesMapper), delegates.actualOffset),
	...
	this.getNextForgers = function (temp, error, success) {
		request.get({
			url: `${app.get('lisk address')}/delegates/forgers?limit=101&offset=0`,
			json: true,
		}, (err, response, body) => {
			if (err || response.statusCode !== 200) {
				return error({ success: false, error: (err || 'Response was unsuccessful') });
