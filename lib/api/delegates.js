...
module.exports = function (app) {
	...
	this.getStandby = function (n, error, success) {
		...
		request.get({
			...
			if (err || response.statusCode !== 200) {
				...
				const bodyDelegates = parseDelegates(body.data.map(delegatesMapper));
				// body.totalCount -= 101;
				// body.pagination = delegates.pagination(body.meta.limit);

				// body.totalCount = body.count - 101;
				// delete body.count;
				// body.pagination = body.pagination(body.meta.limit);
				...
				return success({
					success: true,
					delegates: parseDelegates(bodyDelegates),
					totalCount: body.meta.offset - delegates.actualOffset,
	...
	this.getLastBlocks = function (params, error, success) {
		...
		return request.get({
			...
			body.blocks = _.isArray(body.data) ? body.data : [];
