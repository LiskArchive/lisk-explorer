...
module.exports = function (app) {
    ...
    this.getDelegateByName = function (params, error, success) {
        if (!params.name) {
            return error({ success : false, error : 'Missing/Invalid delegate name' });
        }
        app.db.oneOrNone (sql.getDelegateByName, {delegate: params.name})
            .then(function (result) {
                if (result) {
                    success({success: true, delegate: result});
                } else {
                    error({success: false, error: 'No delegate found'});
                }
            })
            .catch(function (err) {
                error({success: false, error: (err && err.message) || 'Unable to delegate'});
            });
    };

    this.getVotersHistory = function (params, error, success) {
        if (!params.publicKey) {
            return error({success: false, error: 'Missing/Invalid publicKey parameter'});
        }
        app.db.any(sql.getVotersHistory(), params).then(function (result) {
            var cum_balance = 0;
            result = _.map(result, function (row, index) {
                cum_balance = (row.type === 'vote_add' ? (cum_balance + Number(row.balance)) : (row.type === 'vote_remove' ? (cum_balance - Number(row.balance)) : cum_balance));
                ...
                return {
                    ...
                    knowledge: app.knownAddresses.inAddress (row.sender)
                };
            });
            result = result.reverse ();

            success({success: true, history: result, count: result.length});
        })
        .catch(function (err) {
            error({success: false, error: (err && err.message) || 'Unable to get votes history'});
        });
    };
