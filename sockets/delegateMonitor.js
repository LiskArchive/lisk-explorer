...
module.exports = function (app, connectionHandler, socket) {
	let delegates = new api.delegates(app),
	...
	var getActive = function (cb) {
		...
		delegates.getActive(
			(res) => { running.getActive = false; cb('Active'); },
			(res) => { running.getActive = false; cb(null, res); });
	};

	const findActive = function (delegate) {
		return _.find(data.active.delegates, d => d.publicKey === delegate.publicKey);
	};

	var findActiveByPublicKey = function (publicKey) {
		return _.find(data.active.delegates, d => d.publicKey === publicKey);
	};

	const findActiveByBlock = function (block) {
		return _.find(data.active.delegates, d => d.publicKey === block.generatorPublicKey);
	};
	...
	var getRegistrations = function (cb) {
		...
		delegates.getLatestRegistrations(
			(res) => { running.getRegistrations = false; cb('Registrations'); },
			(res) => { running.getRegistrations = false; cb(null, res); });
	};
	...
	var getVotes = function (cb) {
		...
		delegates.getLatestVotes(
			(res) => { running.getVotes = false; cb('Votes'); },
			(res) => { running.getVotes = false; cb(null, res); });
	};
	...
	var getNextForgers = function (cb) {
		...
		delegates.getNextForgers(
			(res) => { running.getNextForgers = false; cb('NextForgers'); },
			(res) => { running.getNextForgers = false; cb(null, res); });
	};
	...
	var getRoundDelegates = function (delegates, height) {
		...
		const filtered = delegates.filter((delegate, index) => currentRound === getRound(height + index + 1));
